...
dev_poll_reactor::dev_poll_reactor(tiny_rpc_asio::io_service& io_service)
  : tiny_rpc_asio::detail::service_base<dev_poll_reactor>(io_service),
...
void dev_poll_reactor::fork_service(tiny_rpc_asio::io_service::fork_event fork_ev)
{
...
{
  tiny_rpc_asio::detail::mutex::scoped_lock lock(mutex_);
  cancel_ops_unlocked(descriptor, tiny_rpc_asio::error::operation_aborted);
}
...
void dev_poll_reactor::run(bool block, op_queue<operation>& ops)
{
  ...
  if (events_size > 0)
  {
    ...
    {
      tiny_rpc_asio::error_code ec = tiny_rpc_asio::error_code(
...
int dev_poll_reactor::do_dev_poll_create()
{
  ...
  if (fd == -1)
  {
    tiny_rpc_asio::error_code ec(errno,
        tiny_rpc_asio::error::get_system_category());
    tiny_rpc_asio::detail::throw_error(ec, "/dev/poll");
  }
  return fd;
}
