...
signal_set_service::signal_set_service(
  ...
  : io_service_(tiny_rpc_asio::use_service<io_service_impl>(io_service)),
#if !defined(ASIO_WINDOWS) \
  && !defined(ASIO_WINDOWS_RUNTIME) \
  && !defined(__CYGWIN__)
    reactor_(tiny_rpc_asio::use_service<reactor>(io_service)),
#endif // !defined(ASIO_WINDOWS)
       //   && !defined(ASIO_WINDOWS_RUNTIME)
       //   && !defined(__CYGWIN__)
    next_(0),
...
void signal_set_service::fork_service(
    tiny_rpc_asio::io_service::fork_event fork_ev)
{
#if !defined(ASIO_WINDOWS) \
  && !defined(ASIO_WINDOWS_RUNTIME) \
  && !defined(__CYGWIN__)
  ...
  switch (fork_ev)
  {
  case tiny_rpc_asio::io_service::fork_prepare:
    {
    ...
    break;
  case tiny_rpc_asio::io_service::fork_parent:
    ...
    break;
  case tiny_rpc_asio::io_service::fork_child:
    if (state->fork_prepared_)
    {
      tiny_rpc_asio::detail::signal_blocker blocker;
      close_descriptors();
      open_descriptors();
      int read_descriptor = state->read_descriptor_;
      state->fork_prepared_ = false;
...
tiny_rpc_asio::error_code signal_set_service::add(
    signal_set_service::implementation_type& impl,
    int signal_number, tiny_rpc_asio::error_code& ec)
{
...
tiny_rpc_asio::error_code signal_set_service::remove(
    signal_set_service::implementation_type& impl,
    int signal_number, tiny_rpc_asio::error_code& ec)
{
  // Check that the signal number is valid.
...
tiny_rpc_asio::error_code signal_set_service::clear(
    signal_set_service::implementation_type& impl,
...
tiny_rpc_asio::error_code signal_set_service::cancel(
    signal_set_service::implementation_type& impl,
    tiny_rpc_asio::error_code& ec)
{
  ...
  op_queue<operation> ops;
  {
    ...
    {
      op->ec_ = tiny_rpc_asio::error::operation_aborted;
      impl.queue_.pop();
      ops.push(op);
    }
  }
...
void signal_set_service::open_descriptors()
{
  ...
  {
    tiny_rpc_asio::error_code ec(errno,
        tiny_rpc_asio::error::get_system_category());
    tiny_rpc_asio::detail::throw_error(ec, "signal_set_service pipe");
  }
#endif // !defined(ASIO_WINDOWS)
       //   && !defined(ASIO_WINDOWS_RUNTIME)
       //   && !defined(__CYGWIN__)
