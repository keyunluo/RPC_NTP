...
{
public:
  explicit overlapped_wrapper(tiny_rpc_asio::error_code& ec)
  {
    Internal = 0;
    InternalHigh = 0;
    Offset = 0;
    ...
    else
    {
      ...
      ec = tiny_rpc_asio::error_code(last_error,
...
win_iocp_handle_service::win_iocp_handle_service(
  ...
  : iocp_service_(tiny_rpc_asio::use_service<win_iocp_io_service>(io_service)),
    mutex_(),
    impl_list_(0)
{
}
...
tiny_rpc_asio::error_code win_iocp_handle_service::assign(
    win_iocp_handle_service::implementation_type& impl,
    const native_handle_type& handle, tiny_rpc_asio::error_code& ec)
{
  if (is_open(impl))
  {
    ec = tiny_rpc_asio::error::already_open;
    return ec;
  }
...
tiny_rpc_asio::error_code win_iocp_handle_service::close(
    win_iocp_handle_service::implementation_type& impl,
...
tiny_rpc_asio::error_code win_iocp_handle_service::cancel(
    win_iocp_handle_service::implementation_type& impl,
    tiny_rpc_asio::error_code& ec)
{
  if (!is_open(impl))
...
size_t win_iocp_handle_service::do_write(
    ...
    const tiny_rpc_asio::const_buffer& buffer, tiny_rpc_asio::error_code& ec)
{
  if (!is_open(impl))
  {
    ec = tiny_rpc_asio::error::bad_descriptor;
  ...
  BOOL ok = ::WriteFile(impl.handle_,
      tiny_rpc_asio::buffer_cast<LPCVOID>(buffer),
      static_cast<DWORD>(tiny_rpc_asio::buffer_size(buffer)), 0, &overlapped);
  if (!ok) 
  {
    DWORD last_error = ::GetLastError();
...
void win_iocp_handle_service::start_write_op(
    win_iocp_handle_service::implementation_type& impl, uint64_t offset,
    const tiny_rpc_asio::const_buffer& buffer, operation* op)
{
  ...
  if (!is_open(impl))
  {
    iocp_service_.on_completion(op, tiny_rpc_asio::error::bad_descriptor);
  }
  else if (tiny_rpc_asio::buffer_size(buffer) == 0)
  {
    // A request to write 0 bytes on a handle is a no-op.
  ...
  else
  {
    ...
    BOOL ok = ::WriteFile(impl.handle_,
        tiny_rpc_asio::buffer_cast<LPCVOID>(buffer),
        static_cast<DWORD>(tiny_rpc_asio::buffer_size(buffer)),
...
size_t win_iocp_handle_service::do_read(
    win_iocp_handle_service::implementation_type& impl, uint64_t offset,
    const tiny_rpc_asio::mutable_buffer& buffer, tiny_rpc_asio::error_code& ec)
{
  if (!is_open(impl))
  {
    ec = tiny_rpc_asio::error::bad_descriptor;
  ...
  BOOL ok = ::ReadFile(impl.handle_,
      tiny_rpc_asio::buffer_cast<LPVOID>(buffer),
      static_cast<DWORD>(tiny_rpc_asio::buffer_size(buffer)), 0, &overlapped);
  if (!ok) 
  {
    DWORD last_error = ::GetLastError();
...
void win_iocp_handle_service::start_read_op(
    win_iocp_handle_service::implementation_type& impl, uint64_t offset,
    const tiny_rpc_asio::mutable_buffer& buffer, operation* op)
{
  update_cancellation_thread_id(impl);
  iocp_service_.work_started();
