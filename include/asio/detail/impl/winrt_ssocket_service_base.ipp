...
void winrt_ssocket_service_base::shutdown_service()
{
  ...
  tiny_rpc_asio::detail::mutex::scoped_lock lock(mutex_);
    ...
    winrt_ssocket_service_base::base_implementation_type& other_impl)
{
  ...
  {
    // Insert implementation into linked list of all implementations.
    tiny_rpc_asio::detail::mutex::scoped_lock lock(other_service.mutex_);
...
tiny_rpc_asio::error_code winrt_ssocket_service_base::close(
    winrt_ssocket_service_base::base_implementation_type& impl,
...
std::size_t winrt_ssocket_service_base::do_get_endpoint(
    ...
    void* addr, std::size_t addr_len, tiny_rpc_asio::error_code& ec) const
{
  if (!is_open(impl))
  {
...
tiny_rpc_asio::error_code winrt_ssocket_service_base::do_set_option(
    winrt_ssocket_service_base::base_implementation_type& impl,
...
void winrt_ssocket_service_base::do_get_option(
    ...
    std::size_t* optlen, tiny_rpc_asio::error_code& ec) const
{
  if (!is_open(impl))
  {
    ec = tiny_rpc_asio::error::bad_descriptor;
...
tiny_rpc_asio::error_code winrt_ssocket_service_base::do_connect(
    winrt_ssocket_service_base::base_implementation_type& impl,
    const void* addr, tiny_rpc_asio::error_code& ec)
{
  if (!is_open(impl))
  {
    ec = tiny_rpc_asio::error::bad_descriptor;
    ...
    const void* addr, winrt_async_op<void>* op, bool is_continuation)
{
  if (!is_open(impl))
  {
    op->ec_ = tiny_rpc_asio::error::bad_descriptor;
  ...
  case ASIO_OS_DEF(AF_INET6):
    ...
    break;
  default:
    op->ec_ = tiny_rpc_asio::error::address_family_not_supported;
    break;
  }
  ...
  {
    op->ec_ = tiny_rpc_asio::error_code(
        e->HResult, tiny_rpc_asio::system_category());
    io_service_.post_immediate_completion(op, is_continuation);
  }
}
    ...
    socket_base::message_flags flags, tiny_rpc_asio::error_code& ec)
{
  ...
  try
  {
    buffer_sequence_adapter<tiny_rpc_asio::const_buffer,
      tiny_rpc_asio::const_buffers_1> bufs(tiny_rpc_asio::buffer(data));
...
void winrt_ssocket_service_base::start_send_op(
      winrt_ssocket_service_base::base_implementation_type& impl,
      const tiny_rpc_asio::const_buffer& data, socket_base::message_flags flags,
      winrt_async_op<unsigned int>* op, bool is_continuation)
{
  if (flags)
  {
    op->ec_ = tiny_rpc_asio::error::operation_not_supported;
    io_service_.post_immediate_completion(op, is_continuation);
    return;
  }
  ...
  {
    op->ec_ = tiny_rpc_asio::error_code(e->HResult,
        tiny_rpc_asio::system_category());
    io_service_.post_immediate_completion(op, is_continuation);
  }
}
    ...
    socket_base::message_flags flags, tiny_rpc_asio::error_code& ec)
{
  ...
  try
  {
    buffer_sequence_adapter<tiny_rpc_asio::mutable_buffer,
        tiny_rpc_asio::mutable_buffers_1> bufs(tiny_rpc_asio::buffer(data));
...
void winrt_ssocket_service_base::start_receive_op(
      winrt_ssocket_service_base::base_implementation_type& impl,
      const tiny_rpc_asio::mutable_buffer& data, socket_base::message_flags flags,
