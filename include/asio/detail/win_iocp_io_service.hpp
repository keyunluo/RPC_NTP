...
class win_iocp_io_service
  : public tiny_rpc_asio::detail::service_base<win_iocp_io_service>
{
public:

  // Constructor. Specifies a concurrency hint that is passed through to the
  // underlying I/O completion port.
  ASIO_DECL win_iocp_io_service(tiny_rpc_asio::io_service& io_service,
  ...
  ASIO_DECL tiny_rpc_asio::error_code register_handle(
      HANDLE handle, tiny_rpc_asio::error_code& ec);

  // Run the event loop until stopped or no more work.
  ASIO_DECL size_t run(tiny_rpc_asio::error_code& ec);

  // Run until stopped or one operation is performed.
  ASIO_DECL size_t run_one(tiny_rpc_asio::error_code& ec);

  // Poll for operations without blocking.
  ASIO_DECL size_t poll(tiny_rpc_asio::error_code& ec);

  // Poll for one operation without blocking.
  ASIO_DECL size_t poll_one(tiny_rpc_asio::error_code& ec);
  ...
  // starting the operation.
  ASIO_DECL void on_completion(win_iocp_operation* op,
      const tiny_rpc_asio::error_code& ec, DWORD bytes_transferred = 0);
...
private:
  ...
  // either 0 or 1).
  ASIO_DECL size_t do_one(bool block, tiny_rpc_asio::error_code& ec);
