...
public:
  ...
  ASIO_DECL tiny_rpc_asio::error_code open(implementation_type& impl,
      const std::string& device, tiny_rpc_asio::error_code& ec);

  // Assign a native handle to a serial port implementation.
  tiny_rpc_asio::error_code assign(implementation_type& impl,
      const native_handle_type& handle, tiny_rpc_asio::error_code& ec)
  {
  ...
  tiny_rpc_asio::error_code set_option(implementation_type& impl,
      const SettableSerialPortOption& option, tiny_rpc_asio::error_code& ec)
  {
  ...
  tiny_rpc_asio::error_code get_option(const implementation_type& impl,
      GettableSerialPortOption& option, tiny_rpc_asio::error_code& ec) const
  {
  ...
  tiny_rpc_asio::error_code send_break(implementation_type&,
      tiny_rpc_asio::error_code& ec)
  {
  ...
  size_t read_some(implementation_type& impl,
      const MutableBufferSequence& buffers, tiny_rpc_asio::error_code& ec)
  {
...
private:
  ...
  typedef tiny_rpc_asio::error_code (*store_function_type)(
      const void*, ::DCB&, tiny_rpc_asio::error_code&);

  // Helper function template to store a serial port option.
  template <typename SettableSerialPortOption>
  static tiny_rpc_asio::error_code store_option(const void* option,
      ::DCB& storage, tiny_rpc_asio::error_code& ec)
  {
  ...
  ASIO_DECL tiny_rpc_asio::error_code do_set_option(
      ...
      const void* option, tiny_rpc_asio::error_code& ec);

  // Function pointer type for loading a serial port option.
  typedef tiny_rpc_asio::error_code (*load_function_type)(
      void*, const ::DCB&, tiny_rpc_asio::error_code&);

  // Helper function template to load a serial port option.
  template <typename GettableSerialPortOption>
  static tiny_rpc_asio::error_code load_option(void* option,
      const ::DCB& storage, tiny_rpc_asio::error_code& ec)
  {
    return static_cast<GettableSerialPortOption*>(option)->load(storage, ec);
  }

  // Helper function to get a serial port option.
  ASIO_DECL tiny_rpc_asio::error_code do_get_option(
      const implementation_type& impl, load_function_type load,
      void* option, tiny_rpc_asio::error_code& ec) const;
