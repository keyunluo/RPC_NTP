...
namespace detail
{
  struct default_connect_condition
  {
    ...
    Iterator operator()(const tiny_rpc_asio::error_code&, Iterator next)
    {
      return next;
    }
  };
    ...
    tiny_rpc_asio::error_code& ec)
{
  ...
  if (!ec)
    ec = tiny_rpc_asio::error::not_found;
...
namespace detail
{
  ...
  protected:
    ...
    template <typename Iterator>
    void check_condition(const tiny_rpc_asio::error_code& ec,
  ...
  protected:
    ...
    template <typename Iterator>
    void check_condition(const tiny_rpc_asio::error_code&, Iterator&, Iterator&)
    {
    }
  };
  ...
  public:
    ...
    void operator()(tiny_rpc_asio::error_code ec, int start = 0)
    {
      switch (start_ = start)
      {
        case 1:
        ...
        handler_(static_cast<const tiny_rpc_asio::error_code&>(ec),
    ...
    Iterator begin, ASIO_MOVE_ARG(ComposedConnectHandler) handler)
{
  ...
  detail::async_result_init<ComposedConnectHandler,
    void (tiny_rpc_asio::error_code, Iterator)> init(
  ...
  detail::connect_op<Protocol, SocketService, Iterator,
    detail::default_connect_condition, ASIO_HANDLER_TYPE(
      ComposedConnectHandler, void (tiny_rpc_asio::error_code, Iterator))>(s,
        begin, Iterator(), detail::default_connect_condition(), init.handler)(
          tiny_rpc_asio::error_code(), 1);
