...
namespace detail {
  ...
  public:
    ...
    {
      *ec_ = tiny_rpc_asio::error_code();
      *value_ = ASIO_MOVE_CAST(T)(value);
      if (--*ready_ == 0)
        (*coro_)();
    }

    void operator()(tiny_rpc_asio::error_code ec, T value)
    {
      *ec_ = ec;
      *value_ = ASIO_MOVE_CAST(T)(value);
      if (--*ready_ == 0)
  ...
  public:
    ...
    void operator()(tiny_rpc_asio::error_code ec)
    {
      *ec_ = ec;
      if (--*ready_ == 0)
        (*coro_)();
...
struct handler_type<basic_yield_context<Handler>,
    ReturnType(tiny_rpc_asio::error_code)>
{
  typedef detail::coro_handler<Handler, void> type;
};
...
struct handler_type<basic_yield_context<Handler>,
    ReturnType(tiny_rpc_asio::error_code, Arg2)>
{
...
public:
  ...
  type get()
  {
    ...
    if (--ready_ != 0)
      ca_();
    if (!out_ec_ && ec_) throw tiny_rpc_asio::system_error(ec_);
...
private:
  ...
  tiny_rpc_asio::error_code* out_ec_;
  tiny_rpc_asio::error_code ec_;
  type value_;
};
...
void spawn(tiny_rpc_asio::io_service::strand strand,
    ASIO_MOVE_ARG(Function) function,
    const boost::coroutines::attributes& attributes)
{
  tiny_rpc_asio::spawn(strand.wrap(&detail::default_spawn_handler),
      ASIO_MOVE_CAST(Function)(function), attributes);
}

template <typename Function>
void spawn(tiny_rpc_asio::io_service& io_service,
    ASIO_MOVE_ARG(Function) function,
    const boost::coroutines::attributes& attributes)
{
  tiny_rpc_asio::spawn(tiny_rpc_asio::io_service::strand(io_service),
      ASIO_MOVE_CAST(Function)(function), attributes);
}
