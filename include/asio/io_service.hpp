 ...
 *
 * @li tiny_rpc_asio::ip::tcp::socket
 * @li tiny_rpc_asio::ip::tcp::acceptor
 * @li tiny_rpc_asio::ip::udp::socket
 * @li tiny_rpc_asio::deadline_timer.
 *
 ...
 *
 * @code
 * tiny_rpc_asio::io_service io_service;
 * ...
 * for (;;)
 * {
 *   try
 ...
 * creating an object of type tiny_rpc_asio::io_service::work:
 *
 * @code tiny_rpc_asio::io_service io_service;
 * tiny_rpc_asio::io_service::work work(io_service);
 * ... @endcode
 *
 ...
 *
 * @code tiny_rpc_asio::io_service io_service;
 * auto_ptr<tiny_rpc_asio::io_service::work> work(
 *     new tiny_rpc_asio::io_service::work(io_service));
 * ...
...
public:
   ...
   * without blocking.
   */
  ASIO_DECL std::size_t run(tiny_rpc_asio::error_code& ec);
   ...
   * @return The number of handlers that were executed.
   */
  ASIO_DECL std::size_t run_one(tiny_rpc_asio::error_code& ec);
   ...
   * @return The number of handlers that were executed.
   */
  ASIO_DECL std::size_t poll(tiny_rpc_asio::error_code& ec);
   ...
   * @return The number of handlers that were executed.
   */
  ASIO_DECL std::size_t poll_one(tiny_rpc_asio::error_code& ec);
   ...
   *
   * @throws tiny_rpc_asio::system_error Thrown on failure. If the notification
   ...
   * function:
   * @code my_io_service.notify_fork(tiny_rpc_asio::io_service::fork_prepare);
   * if (fork() == 0)
   * {
   *   // This is the child process.
   *   my_io_service.notify_fork(tiny_rpc_asio::io_service::fork_child);
   * }
   * else
   * {
   *   // This is the parent process.
   *   my_io_service.notify_fork(tiny_rpc_asio::io_service::fork_parent);
   * } @endcode
   *
   ...
   */
  ASIO_DECL void notify_fork(tiny_rpc_asio::io_service::fork_event event);
   ...
   *
   * @throws tiny_rpc_asio::service_already_exists Thrown if a service of the
   * given type is already present in the io_service.
   *
   * @throws tiny_rpc_asio::invalid_service_owner Thrown if the service's owning
...
private:
  ...
  tiny_rpc_asio::detail::service_registry* service_registry_;
...
public:
   ...
   */
  explicit work(tiny_rpc_asio::io_service& io_service);
  ...
  tiny_rpc_asio::io_service& get_io_service();
...
protected:
   ...
   */
  ASIO_DECL service(tiny_rpc_asio::io_service& owner);
...
private:
  ...
  ASIO_DECL virtual void fork_service(
      tiny_rpc_asio::io_service::fork_event event);

  friend class tiny_rpc_asio::detail::service_registry;
  struct key
  {
    key() : type_info_(0), id_(0) {}
    const std::type_info* type_info_;
    const tiny_rpc_asio::io_service::id* id_;
  } key_;

  tiny_rpc_asio::io_service& owner_;
  service* next_;
};
...
class service_id
  : public tiny_rpc_asio::io_service::id
{
};
...
{
public:
  static tiny_rpc_asio::detail::service_id<Type> id;

  // Constructor.
  service_base(tiny_rpc_asio::io_service& io_service)
    : tiny_rpc_asio::io_service::service(io_service)
  {
  }
};

template <typename Type>
tiny_rpc_asio::detail::service_id<Type> service_base<Type>::id;
