...
public:
   ...
   * } @endcode
   */
  basic_yield_context operator[](tiny_rpc_asio::error_code& ec) const
  {
...
/**
 * @defgroup spawn tiny_rpc_asio::spawn
 *
 ...
 *
 * @code tiny_rpc_asio::spawn(my_strand, do_echo);
 *
 * // ...
 *
 * void do_echo(tiny_rpc_asio::yield_context yield)
 * {
 *   try
 *   {
 *     char data[128];
 ...
 *           tiny_rpc_asio::buffer(data), yield);
 *
 *       tiny_rpc_asio::async_write(my_socket,
 *           tiny_rpc_asio::buffer(data, length), yield);
 *     }
 *   }
