...
public:
  ...
  ASIO_DECL tiny_rpc_asio::error_code set_verify_callback(
  ...
  ASIO_DECL want shutdown(tiny_rpc_asio::error_code& ec);

  // Write bytes to the SSL session.
  ASIO_DECL want write(const tiny_rpc_asio::const_buffer& data,
      tiny_rpc_asio::error_code& ec, std::size_t& bytes_transferred);

  // Read bytes from the SSL session.
  ASIO_DECL want read(const tiny_rpc_asio::mutable_buffer& data,
      tiny_rpc_asio::error_code& ec, std::size_t& bytes_transferred);

  // Get output data to be written to the transport.
  ASIO_DECL tiny_rpc_asio::mutable_buffers_1 get_output(
      const tiny_rpc_asio::mutable_buffer& data);

  // Put input data that was read from the transport.
  ASIO_DECL tiny_rpc_asio::const_buffer put_input(
      const tiny_rpc_asio::const_buffer& data);

  // Map an error::eof code returned by the underlying transport according to
  // the type and state of the SSL session. Returns a const reference to the
  // error code object, suitable for passing to a completion handler.
  ASIO_DECL const tiny_rpc_asio::error_code& map_error_code(
...
private:
  ...
  ASIO_DECL static tiny_rpc_asio::detail::static_mutex& accept_mutex();
