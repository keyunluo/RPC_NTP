...
typedef boost::function<void (const tiny_rpc_asio::error_code&, int)>
  user_handler_func;
...
public:
  ...
  openssl_operation(ssl_primitive_func primitive,
                    ...
                    tiny_rpc_asio::io_service::strand& strand
                    )
  ...
  int start()
  {
    ...
    if (error_code == SSL_ERROR_SSL)
      return handler_(tiny_rpc_asio::error_code(
            sys_error_code, tiny_rpc_asio::error::get_ssl_category()), rc);
      ...
      && !is_shut_down_sent)
    {
      ...
      {
        return handler_(tiny_rpc_asio::error_code(
              sys_error_code, tiny_rpc_asio::error::system_category), rc);
      }
      else
      {
        return handler_(tiny_rpc_asio::error_code(
      ...
      if (recv_buf_.get_data_len() > 0)
      {
        ...
        else if (written < 0)
        {
          if (!BIO_should_retry(ssl_bio_))
          {
            ...
            return handler_(tiny_rpc_asio::error::no_recovery, 0);
          }
        }
...
private:
  typedef boost::function<int (const tiny_rpc_asio::error_code&, int)>
    int_handler_func;
  ...
  //
  int sync_user_handler(const tiny_rpc_asio::error_code& error, int rc)
  {
    if (!error)
      return rc;

    throw tiny_rpc_asio::system_error(error);
  }
    
  int async_user_handler(tiny_rpc_asio::error_code error, int rc)
  {
    if (rc < 0)
    {
      if (!error)
        error = tiny_rpc_asio::error::no_recovery;
      rc = 0;
    }
  ...
  int  do_async_write(bool is_operation_done, int rc) 
  {
    ...
    { 
      ...
      if (len > 0)
      {
        ...
        ASIO_ASSERT(strand_);
        tiny_rpc_asio::async_write
        ( 
          socket_, 
          tiny_rpc_asio::buffer(data_start, len),
          strand_->wrap
          (
            boost::bind
            (
              ...
              is_operation_done,
              rc, 
              tiny_rpc_asio::placeholders::error,
              tiny_rpc_asio::placeholders::bytes_transferred
            )
          )
        );
      ...
      else if (!BIO_should_retry(ssl_bio_))
      {
        ...
        handler_(tiny_rpc_asio::error::no_recovery, 0);
        return 0;
      }
    }
    ...
    if (is_operation_done)
    {
      ...
      handler_(tiny_rpc_asio::error_code(), rc);
      return 0;
    }
  ...
  void async_write_handler(bool is_operation_done, int rc, 
    const tiny_rpc_asio::error_code& error, size_t bytes_sent)
  {
    if (!error)
    {
  ...
  int do_async_read()
  {
    ...
    ( 
      tiny_rpc_asio::buffer(recv_buf_.get_unused_start(),
  ...
  void async_read_handler(const tiny_rpc_asio::error_code& error,
      size_t bytes_recvd)
  {
    if (!error)
    {
  ...
  int do_sync_write(bool is_operation_done, int rc)
  {
    ...
    { 
      ...
      if (len > 0)
      {
        size_t sent_len = tiny_rpc_asio::write(
                  socket_, 
                  tiny_rpc_asio::buffer(send_buf_.get_unused_start(), len)
                  );
      ...
      else if (!BIO_should_retry(ssl_bio_))
      {
        // Seems like fatal error
        // reading from SSL BIO has failed...
        throw tiny_rpc_asio::system_error(tiny_rpc_asio::error::no_recovery);
      }
    }
