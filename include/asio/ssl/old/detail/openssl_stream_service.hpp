...
class openssl_stream_service
  : public tiny_rpc_asio::detail::service_base<openssl_stream_service>
{
private:
  ...
  public:
    typedef boost::function<
      void (const tiny_rpc_asio::error_code&, size_t)> func_t;

    base_handler(tiny_rpc_asio::io_service& io_service)
      : op_(NULL)
      , io_service_(io_service)
      , work_(io_service)
    {}
    
    void do_func(const tiny_rpc_asio::error_code& error, size_t size)
    {
      func_(error, size);
    }
  ...
  private:
    ...
    tiny_rpc_asio::io_service& io_service_;
    tiny_rpc_asio::io_service::work work_;
  };  // class base_handler
  ...
  {
  public:
    io_handler(Handler handler, tiny_rpc_asio::io_service& io_service)
  ...
  private:
    Handler handler_;
    void handler_impl(const tiny_rpc_asio::error_code& error, size_t size)
    {
  ...
  {
  public:
    handshake_handler(Handler handler, tiny_rpc_asio::io_service& io_service)
      : base_handler<Stream>(io_service)
      , handler_(handler)
    {
      this->set_func(boost::bind(
  ...
  private:
    Handler handler_;
    void handler_impl(const tiny_rpc_asio::error_code& error, size_t)
    {
  ...
  {
  public:
    shutdown_handler(Handler handler, tiny_rpc_asio::io_service& io_service)
      : base_handler<Stream>(io_service),
        handler_(handler)
    { 
      this->set_func(boost::bind(
...
public:
  ...
  explicit openssl_stream_service(tiny_rpc_asio::io_service& io_service)
    : tiny_rpc_asio::detail::service_base<openssl_stream_service>(io_service),
      strand_(io_service)
  {
  }
  ...
  template <typename Stream>
  tiny_rpc_asio::error_code handshake(impl_type& impl, Stream& next_layer,
      stream_base::handshake_type type, tiny_rpc_asio::error_code& ec)
  {
    try
    {
    ...
    catch (tiny_rpc_asio::system_error& e)
    {
      ec = e.code();
      return ec;
    }
  ...
  template <typename Stream>
  tiny_rpc_asio::error_code shutdown(impl_type& impl, Stream& next_layer,
  ...
  std::size_t write_some(impl_type& impl, Stream& next_layer,
      const Const_Buffers& buffers, tiny_rpc_asio::error_code& ec)
  {
    size_t bytes_transferred = 0;
    try
    {
      tiny_rpc_asio::const_buffer buffer =
        tiny_rpc_asio::detail::buffer_sequence_adapter<
          tiny_rpc_asio::const_buffer, Const_Buffers>::first(buffers);

      std::size_t buffer_size = tiny_rpc_asio::buffer_size(buffer);
      ...
      const Const_Buffers& buffers, Handler handler)
  {
    ...
    {
      get_io_service().post(tiny_rpc_asio::detail::bind_handler(
            handler, tiny_rpc_asio::error_code(), 0));
      return;
    }
  ...
  std::size_t read_some(impl_type& impl, Stream& next_layer,
      const Mutable_Buffers& buffers, tiny_rpc_asio::error_code& ec)
  {
    size_t bytes_transferred = 0;
    try
    {
      tiny_rpc_asio::mutable_buffer buffer =
        tiny_rpc_asio::detail::buffer_sequence_adapter<
          tiny_rpc_asio::mutable_buffer, Mutable_Buffers>::first(buffers);
      ...
      boost::function<int (SSL*)> recv_func =
        boost::bind(boost::type<int>(), &::SSL_read, boost::arg<1>(),
            tiny_rpc_asio::buffer_cast<void*>(buffer),
            static_cast<int>(buffer_size));
...
private:  
  tiny_rpc_asio::io_service::strand strand_;

  typedef tiny_rpc_asio::detail::mutex mutex_type;
