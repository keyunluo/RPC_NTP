 ...
 * tiny_rpc_asio::ssl::context context(io_service, tiny_rpc_asio::ssl::context::sslv23);
 * tiny_rpc_asio::ssl::stream<tiny_rpc_asio::ip::tcp::socket> sock(io_service, context);
 * @endcode
 *
 * @par Concepts:
...
public:
  ...
  explicit stream(Arg& arg, basic_context<Context_Service>& context)
    : next_layer_(arg),
      service_(tiny_rpc_asio::use_service<Service>(next_layer_.get_io_service())),
      impl_(service_.null())
  {
   ...
   * @returns The number of bytes of data that can be read without blocking.
   */
  std::size_t in_avail(tiny_rpc_asio::error_code& ec)
  {
