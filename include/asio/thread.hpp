...
/**
 * The tiny_rpc_asio::thread class implements the smallest possible subset of the
 ...
 *
 * @par Example
 * A typical use of tiny_rpc_asio::thread would be to launch a thread to run an
 * io_service's event processing loop:
 *
 * @par
 * @code tiny_rpc_asio::io_service io_service;
 * // ...
 * tiny_rpc_asio::thread t(boost::bind(&tiny_rpc_asio::io_service::run, &io_service));
 * // ...
 * t.join(); @endcode
 */
class thread
